...
class WGAN(BasicGAN):
    def __init__(self, cfg):
        ...
        # self.d_optimizer = torch.optim.SGD(self.D.parameters(), lr=cfg.SOLVER.BASE_LR, momentum=cfg.SOLVER.MOMENTUM)
        # self.g_optimizer = torch.optim.SGD(self.G.parameters(), lr=cfg.SOLVER.BASE_LR, momentum=cfg.SOLVER.MOMENTUM)
    ...
    def train(self, train_loader):
        ...
        for g_iter in range(self.generator_iters):
            ...
            for d_iter in range(self.critic_iter):
                ...
                # real_labels = torch.ones(images.size(0)).to(self.device)
                # d_loss_real = self.loss_fn(self.D(images), real_labels)
                # d_loss_real.backward()
                #
                # # Train with fake images
                # z = torch.randn(self.batch_size, self.G_input_size, 1, 1).to(self.device)
                # fake_images = self.G(z)
                # fake_labels = torch.zeros(fake_images.size(0)).to(self.device)
                # d_loss_fake = self.loss_fn(self.D(fake_images), fake_labels)
            ...
            #
            # fake_images = self.G(z)
            # g_loss = self.loss_fn(self.D(fake_images), real_labels)
            # g_loss.backward()
            #
            # g_cost = g_loss
